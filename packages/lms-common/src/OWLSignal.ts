 ...
 *   writes are tracked in side the OWLSignal. If there are multiple writers for the same data (i.e.
  ...
  private async writeLoop() {
    ...
    while (this.queuedUpdates.length > 0) {
      ...
      await new Promise<void>(nextStep => {
        ...
        this.writeUpstream(...updater(this.get() as StripNotAvailable<TData>), [tag, ...tags]);
      });
    }
